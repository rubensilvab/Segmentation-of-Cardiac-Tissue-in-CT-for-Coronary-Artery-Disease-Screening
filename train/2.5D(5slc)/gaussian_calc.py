...
def noisy_gaussian(image,mask):
      ...
      if len(points_mask>0):
          ...
          if random_n==1:
              ...
              gauss2=gauss2.reshape(row,col,1)*mask
              
          else:
              gauss2=0
              
          gauss=makeGaussian(row,center=points_mask[index])
          gauss=gauss.reshape(row,col,1)*mask
          
          gauss_total=gauss + gauss2
          gauss_3s=np.stack([gauss_total,gauss_total, gauss_total,gauss_total,gauss_total], axis=-1).reshape(row,col,5)
          #gauss=gauss.astype(np.uint16)
          noisy = image+gauss_3s
          noisy[noisy>1] = 1
