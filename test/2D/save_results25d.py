...
def pos_process(pred):
    ...
    convex_predict=convex_mask(connected)
    fill_2d_predict=fill_holes(connected)
    
    convex_predict=fill_3d(convex_predict)
    fill_2d_predict=fill_3d(fill_2d_predict)
...
def convert_nrrd(test_dataframe,model,path,Width,Length,change_hu=False):
    ...
    for i in range(len(patients)):
        ...
        nrrd.write(str(patients[i])+'_'+str(Width)+'_UNet'+'.nrrd', reshape_nrrd(pred_test,n),header=header)
        ...
        # nrrd.write(str(patients[i])+'_'+str(Width)+'_2.5UNet+conv+fill3d'+'.nrrd', reshape_nrrd(convex_predict,n),header=header)

        # nrrd.write(str(patients[i])+'_'+str(Width)+'_2.5UNet+fill2d+fill3d'+'.nrrd', reshape_nrrd(fill_2d_predict,n),header=header)
