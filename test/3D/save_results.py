...
def predict(model,X,X_original,Width,Length):
  ...
  #print(pred.shape) 
  zoriginal,woriginal,loriginal=X_original.shape[0],X_original.shape[1],X_original.shape[2]
  
  for i in range(zoriginal):
      predic=cv2.resize(pred[i], (woriginal,loriginal))
      predic=(predic>0.5).astype(np.uint8)
      prediction.append(np.array(predic))
      
  prediction=np.array(prediction).reshape(-1,woriginal,loriginal)
...
"Organizao estrutura para preencher slices"
...
def get_black_image_indices(image_array):
    black_indices = []
...
def remove_black_images(image_array, black_indices):
    non_black_images = np.delete(image_array, black_indices, axis=0)
    return non_black_images

def convert_nrrd(test_dataframe,model,path,Width,Length,name_excel,change_hu=False):
    #Copy header
    if 'ospit' in path:
        print('entrou ceerto')
    ...
    for i in range(len(patients)):
        ...
        batch_size=1
        data_generator=generators(CustomDataGenerator(csv_file,cols, batch_size=batch_size,input_shape=(Width,Length)))
        ...
        X,X_ori, Y,csv_3d= next(data_generator) 
        X_p=np.squeeze(np.array(X))
        X_p_original=np.squeeze(np.array(X_ori))
        Y_p=np.squeeze(np.array(Y))
        
        csv_3d=csv_3d.dropna()
        ...
        #remover slices pretos adicionados no 3d
        black_indices=get_black_image_indices(X_p_original)
        
        X_p_original=remove_black_images(X_p_original, black_indices)
        pred_test=remove_black_images(pred_test, black_indices)
        Y_p=remove_black_images(Y_p, black_indices)

        
        X3D,Y3D,Y_pred_3d=organize_data(csv_file, csv_3d, X_p_original, pred_test, Y_p)
        
        #Connected components and fill slices:
        convex_predict,fill_2d_predict=pos_process(Y_pred_3d)  
        convex_predict,fill_2d_predict=convex_predict.astype(Y_pred_3d.dtype),fill_2d_predict.astype(Y_pred_3d.dtype)
        
        
        #print('y3d:',Y3D.shape,'ypred3d:',Y_pred_3d.shape)
        save_img_results(path,str(patients[i]),X3D,Y3D,Y_pred_3d)
     
        get_excel_row(path,str(patients[i]),name_excel,Y3D,Y_pred_3d)
        ...
        nrrd.write(str(patients[i])+'_'+str(Width)+'_3DNet'+'.nrrd', reshape_nrrd(Y_pred_3d,n),header=header)
    
        nrrd.write(str(patients[i])+'_'+str(Width)+'_manual'+'.nrrd', reshape_nrrd(Y3D,n),header=header)
        
        nrrd.write(str(patients[i])+'_'+str(Width)+'_3DNet+pp+conv'+'.nrrd', reshape_nrrd(convex_predict,n),header=header)

        nrrd.write(str(patients[i])+'_'+str(Width)+'_3DNet+pp+fill2d'+'.nrrd', reshape_nrrd(fill_2d_predict,n),header=header)
