...
def convert_nrrd(test_dataframe,csv_original,model25d,path,Width,Length,name_excel,change_hu=False):
    #Copy header
    if 'ospit' in path:
        print('entrou ceerto')
    ...
    for i in range(len(patients)):
        ...
        csv_original_p=csv_original.loc[csv_original['Patient'].isin([(patients[i])])]
        ...
        batch_size25d=12
        data_generator25d=generators(CustomDataGenerator25d(csv_file,cols,batch_size=batch_size25d,input_shape=(Width,Length)))
        ...
        try: #se csv_to_25d tem mais de 2            
            ...
            X25D,Y25D,Y_pred_25d=organize_data(csv_original_p, csv_file, X_p_original25d, pred_test25d, Y_p25d)
        
        except: #se csv_to_25d tem menos de 2 
            print(str(patients[i]),'não segmentou nada')
            indexes=csv_file.index
            X25D,Y_pred_25d,Y25D=slices_add(csv_original_p,indexes)
        ...
        nrrd.write(str(patients[i])+'_'+str(Width)+'_slc+2.5d'+'.nrrd', reshape_nrrd(Y_pred_25d,n),header=header)
    
        nrrd.write(str(patients[i])+'_'+str(Width)+'_manual'+'.nrrd', reshape_nrrd(Y25D,n),header=header)
        
        nrrd.write(str(patients[i])+'_'+str(Width)+'_slc+2.5d+pp+conv'+'.nrrd', reshape_nrrd(convex_predict,n),header=header)

        #nrrd.write(str(patients[i])+'_'+str(Width)+'_slc+2.5d+pp+fill2d'+'.nrrd', reshape_nrrd(fill_2d_predict,n),header=header)
