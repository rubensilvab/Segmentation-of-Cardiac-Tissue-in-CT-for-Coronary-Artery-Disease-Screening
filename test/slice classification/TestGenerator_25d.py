...
class CustomDataGenerator25d(Sequence):
    
    def __init__(self, csv_file,col, batch_size=12,input_shape=(256, 256), change_hu=False,shuffle=False, seed=None ):
        ...
        self.seed=seed
        self.csv_file=csv_file
        self.list_paths = self.sort_specific(self.csv_file[self.col[0]])
        self.list_masks = self.sort_specific(self.csv_file[self.col[1]])
    ...
    def __getitem__(self, index):
        ...
        for i, idx in enumerate(indexes):
            ...
            input_path, mask_path = row[self.col[0]],row[self.col[1]]
          
            
            
            #Load image
            input_img = cv2.resize(cv2.imread(input_path, flags=cv2.IMREAD_ANYDEPTH), self.input_shape)
            original_input_img=cv2.imread(input_path, flags=cv2.IMREAD_ANYDEPTH) # without resize
            ...
            number_slices=len(all_data)
            
            if idx==0:
                row_top= all_data.iloc[idx+1]       
                path_top,path_bottom=row_top[self.col[0]],input_path
                #top_instance,bottom_instance="{:02d}".format(instance_int+1),"{:02d}".format(instance_int)
                #print('primeiro slice')
               
            elif idx==number_slices-1:     
                row_bottom= all_data.iloc[idx-1]       
                path_top,path_bottom=input_path,row_bottom[self.col[0]]
                #print('ultimo slice')
            
            else:
                row_top,row_bottom= all_data.iloc[idx+1],all_data.iloc[idx-1]  
                path_top,path_bottom=row_top[self.col[0]],row_bottom[self.col[0]]
            ...
            if self.change_hu:
                ...
                original_input_img=self.reconvert_HU(original_input_img,50,350)
            ...
            batch_y.append(mask)
            batch_x_original.append(original_input_img)
            
        return np.array(batch_x),np.array(batch_y),np.array(batch_x_original)
...
Width=256
train_combine_generator=generators(CustomDataGenerator25d(osic_3d_df, cols, batch_size, input_shape=(Width, Width)))
val_combine_generator=generators(CustomDataGenerator25d(val_osic_3d, cols, batch_size, input_shape=(Width, Width)))
